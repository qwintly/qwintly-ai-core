...
export const computeBackoffMs = (attempt: number, baseMs: number, maxMs: number) => {
...
export const aiCallWithRetry = async (params: {
  aiCall: AiCallLike;
  request: unknown;
  options: { tools?: Tool[]; model?: string; toolCallingMode?: FunctionCallingConfigMode };
  retryMax: number;
  retryBaseMs: number;
  retryMaxMs: number;
  step: number;
  ...
  const { aiCall, request, options, retryMax, retryBaseMs, retryMaxMs, step, logger } =
    params;
...
export const recordToolEvent = (params: {
  ...
  const { toolEvents, name, effectiveArgs, modelArgs, readFileMeta, toolResult, toolResultRaw } =
    params;
